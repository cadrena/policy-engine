...
// the driver this package registers as [DriverName] -- the one carrying every
...
//	defer db.Close()
//
// For opening a database this is equivalent to sql.Open(DriverName, dsn). It
...
// [DriverName], so (*sql.DB).Driver() reports the same value for a database
// opened through this Connector as for one opened with sql.Open(DriverName,
// dsn).
